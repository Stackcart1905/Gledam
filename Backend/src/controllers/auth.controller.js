...
const signup = async (req, res) => {
  ...
  // In development mode, auto-verify the user regardless of email sending
  if (process.env.NODE_ENV === "development") {
    user.isVerified = true;
    await user.save();
    // Try to send OTP but don't wait for it
    if (hasEmailCreds) {
      sendOTP(user, "signup").catch(err => {
        console.warn("Signup: failed to send OTP email in development:", err?.message);
      });
    }
    return res.status(201).json({
      success: true,
      message: "User registered. Account verified for development.",
    });
  }
  
  // In production, send OTP if credentials exist
  if (hasEmailCreds) {
    try {
  ...
  // Auto-verify when email sending fails
  if (!hasEmailCreds || !otpSent) {
    user.isVerified = true;
    await user.save();
    return res.status(201).json({
      success: true,
      message: hasEmailCreds && otpSent === false
        ? "User registered. Email send failed; account verified."
        : "User registered. Account verified.",
    });
  }
